...
public class AlphaLoader extends SiteLoader{
    int firstIndexToMinus=19, secondIndexToMinus = 15;
    /**
    ...
    protected double handle(String content, Currency currencyName) {
        ...
        for (int i = 0; i < lines.length; i++) {
            if (lines[i].contains(substring1) && lines[i].contains(substring2)) {
                return Double.parseDouble(lines[i].substring(lines[i].indexOf(substring2) - firstIndexToMinus, lines[i].indexOf(substring2) - secondIndexToMinus));
            }
        }
        return -999;
